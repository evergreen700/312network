...
# Reimplemented to allow setting source/target nodes with mouse click
    def mousePressEvent(self, e):
        scale = self.getScale()	
        if PYQT_VER == 'PYQT6':
            self.pointclicked.emit(self.clicknode, QPointF(
                (e.position().x()-self.width())/scale+2, (self.height()-e.position().y())/scale-1))
        else:
            self.pointclicked.emit(self.clicknode, QPointF(
                (e.pos().x()-self.width())/scale+2, (self.height()-e.pos().y())/scale-1))
    ...
    def generateClicked(self):
        if int(self.size.text()) < 4:
            self.statusBar.showMessage(
                'Input Error: Network size must be greater than 3')
        else:
            self.statusBar.showMessage('')
            self.generateNetwork()
            self.view.addPoints(
                [x.loc for x in self.graph.getNodes()], (0, 0, 0))
            self.view.repaint()
# if self.graph:
# self.generateNetwork()
...
# self.view.repaint()
    ...
    def display_paths(self, heap_path, heap_time, array_path, array_time):
        ...
        if heap_path:
            ...
            for start, end, lbl in heap_path['path']:
                self.view.addEdge(startPt=start, endPt=end,
                                  label=lbl, edgeColor=(128, 128, 255))
    ...
    def computeClicked(self):
        ...
        if self.useUnsorted.isChecked():
            ...
            doHeap = True
        if doArray:
            array_time = self.solver.computeShortestPaths(
                int(self.sourceNode.text())-1, use_heap=False)
            array_path = self.solver.getShortestPath(
                int(self.targetNode.text())-1)
            dist = array_path['cost']
        if doHeap:
            heap_time = self.solver.computeShortestPaths(
                int(self.sourceNode.text())-1, use_heap=True)
            heap_path = self.solver.getShortestPath(
                int(self.targetNode.text())-1)
            dist = heap_path['cost']
        ...
        if not self.graphReady:
            ...
            for node in self.graph.nodes:
                if math.sqrt(pow((abs(node.loc.x()-point.x())), 2) + pow((abs(node.loc.y()-point.y())), 2)) < dist:
                    dist = math.sqrt(
                        pow((abs(node.loc.x()-point.x())), 2) + pow((abs(node.loc.y()-point.y())), 2))
                    id = node.node_id+1
            if id != -1:
                self.view.clearEdges()
